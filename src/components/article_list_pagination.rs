...
pub fn ArticleListWithPagination(props: ArticleListWithPaginationProps) -> Element {
    ...
    // ✅ 修复：使用 use_reactive! 确保响应性
    let articles = use_resource(use_reactive!(|(feed_type, tag, author, favorited, token_clone, page_size, current_page)| async move {
        let current_page_val = current_page;
        
        // ✅ 关键修复：解析 feed_type 以获取标签信息
        if feed_type.starts_with("tag:") {
            let tag_from_feed = feed_type.strip_prefix("tag:").unwrap_or("");
            // 构建请求参数，包含标签过滤
            let query = ArticleQuery {
                tag: Some(tag_from_feed.to_string()),
                author,
                favorited,
                limit: Some(page_size as u32),
                offset: Some(((current_page_val - 1) * page_size) as u32),
            ...
            match feed_type.as_str() {
                "your" if token_clone.is_some() => {
                    // 获取用户关注的文章 - 注意：这里可能需要支持分页参数
                    fetch_user_feed_articles(&token_clone.unwrap()).await.map(|resp| {
                        // 手动处理分页（如果API不支持分页）
                        let start = ((current_page_val - 1) * page_size).min(resp.articles.len());
                        let end = (current_page_val * page_size).min(resp.articles.len());
                        let articles = resp.articles[start..end].to_vec();
                ...
                _ => {
                    // 构建查询参数 - 使用 props.tag 而不是从 feed_type 解析的标签
                    let query = ArticleQuery {
                        tag,
                        author,
                        favorited,
